    ...
    loop {
        ...
        let msg = if wait_for_events {
            match rx.recv() {
                Ok(value) => Some(value),
                Err(_) => {
                    dbg!("MerkleStorage GC thread shut down! reason: mpsc::Sender dropped.");
                    return;
                },
            }
        } else {
        ...
        match msg {
            ...
            None => {
                if received_exit_msg && todo_keys.len() == 0 && reused_keys.len() == len {
                    dbg!("MerkleStorage GC thread shut down! reason: received exit message.");
                    return;
                }
            }
        }
        ...
        if todo_keys.len() > 0 {
            let processed_len = todo_keys.len().min(2048);
