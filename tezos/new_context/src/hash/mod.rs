    ...
    if entries.is_empty() {
        ...
        for i in 0..=31 {
            let entries_at_depth_and_index_i: Vec<(&KeyFragment, &Node)> = entries
                ...
                .cloned()
                .collect();
            let ti = partition_entries(depth + 1, &entries_at_depth_and_index_i, hashes)?;

            match ti {
                Inode::Empty => (),
                non_empty => pointers.push((i as u8, hash_long_inode(&non_empty, hashes)?)),
            }
        }
    ...
    match inode {
        ...
        Inode::Value(entries) => {
            ...
            for (name, node) in entries {
                ...
                hasher.update(node.entry_hash(hashes)?);
            }
        }
        Inode::Tree {
            depth,
...
pub(crate) fn hash_entry(entry: &Entry, hashes: &mut HashInterner) -> Result<HashId, HashingError> {
    match entry {
        ...
        Entry::Blob(blob) => hash_blob(blob, hashes),
    }
}
